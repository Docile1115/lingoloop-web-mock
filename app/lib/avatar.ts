...
/** Returns deterministic SVG markup made exclusively from internal allowlisted fragments. */
export function renderAvatarSvg(value: unknown): string {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128"><rect width="128" height="128" rx="24" fill="${background}"/><circle cx="104" cy="24" r="16" fill="#fff" fill-opacity=".2"/>${outfitShape(config.outfit, outfit)}<rect x="57" y="82" width="14" height="19" rx="7" fill="${skin}"/>${FACE_SHAPES[config.face].replace("/>", ` fill="${skin}"/>`)}<ellipse cx="42" cy="64" rx="4" ry="7" fill="${skin}"/><ellipse cx="86" cy="64" rx="4" ry="7" fill="${skin}"/>${hairShape(config.hair, hair)}${EYES[config.eyes]}<path d="M63 62l-2 8h5" fill="none" stroke="#7e4c3d" stroke-opacity=".45" stroke-width="1.6" stroke-linecap="round"/>${MOUTHS[config.mouth]}${accessoryShape(config.accessory)}</svg>`;
}
