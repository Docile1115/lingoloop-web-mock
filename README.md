...
- 전신 캐릭터는 `profiles/{uid}.avatarConfig`의 version 2 파츠 ID로 저장합니다. 얼굴 6항목·헤어 2항목·의상 4항목·발끝 3항목·소품 4항목을 조합하며, 편집기는 전신/얼굴 확대와 부위별 선택 미리보기를 제공합니다. 마이룸에는 전신, 채팅·피드의 작은 아이콘에는 얼굴 중심 SVG를 사용합니다. 기존 version 1 설정은 읽을 때 기본 하의·신발 등으로 확장하며, 구형 클라이언트가 얼굴을 저장해도 이미 저장된 전신 설정을 덮어쓰지 않습니다. 별도 이미지 업로드나 AI 생성 비용 없이 웹·네이티브가 같은 벡터 렌더러를 공유합니다.
- 마이룸은 `profiles/{uid}.roomConfig`에 저장합니다. 각 가구는 `{ id, x, y, flipped }`이며 5×5 배치 칸과 허용된 종류만 받습니다. 같은 가구·같은 위치의 중복과 캐릭터 자리 점유를 거부합니다. 클라이언트에 포함된 벡터로 렌더링하므로 별도 이미지 저장소나 AI 호출이 필요하지 않습니다. 기존 사용자에게는 기본 인테리어를 표시하고 첫 저장 때 영속화합니다.
...
| `PATCH` | `/api/profile/avatar` | 허용된 파츠 ID로 내 전신 캐릭터 저장·사진 모드 전환. version 2 계약, 기존 version 1 입력 호환 |
| `PATCH` | `/api/profile/room` | 내 방의 벽지·바닥·가구 배치를 검증하고 저장 (`{ config: { version: 1, wall, floor, items } }`) |
